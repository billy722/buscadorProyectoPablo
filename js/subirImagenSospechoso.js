(function(){
    ...
    if(input.addEventListener){
        input.addEventListener('change', function(evt){
            ...
            if(formdata){
                document.getElementById('response').innerHTML = "<h3 class='text-warning'><i class='fa fa-spinner'> </i> Subiendo archivo...<h3>";
                $.ajax({
                    url : 'php/uploadProducto.php?id='+localStorage['id_producto'],
                    type : 'POST',
                    data : formdata,
                    processData : false, 
                    contentType : false, 
                    success : function(res){
                        if(res==0){
                            document.getElementById('response').innerHTML = "<h3 class='text-danger'><i class='fa fa-ban'> </i> Error al subir archivos, porfavor reinicie la página y pruebe nuevamente<h3>";
                        }
                        else if(res==3){
                            document.getElementById('response').innerHTML = "<h3 class='text-danger'><i class='fa fa-ban'> </i> Error al subir archivos......<h3>";
                        }
                        else{
